---
change_id: {{CHANGE_ID}}
title: {{TITLE}}
risk_tier: high-risk
status: draft
date: {{DATE}}
tier_rationale: TBD
affected_paths: TBD
review_owner: TBD
---

# {{TITLE}}

## Goal and non-goals

Goal: TBD

Non-goals: TBD

## Current state and expected outcomes

Current: TBD

| Outcome ID | Observable expected result | Threshold or evidence |
|---|---|---|
| OUT-001 | TBD | TBD |

## Requirements

- REQ-001: TBD

## Acceptance criteria

- AC-001 (REQ-001): Given TBD, when TBD, then TBD.

## Constraints, assumptions, and unknowns

- Constraint: TBD
- Assumption: TBD
- Unknown: TBD

## Options and decision

| Option | Benefit | Cost or risk | Decision |
|---|---|---|---|
| TBD | TBD | TBD | TBD |

## Design and contracts

TBD

## Necessary UML

decision_question: TBD
traces: REQ-001, AC-001

~~~mermaid
sequenceDiagram
    ParticipantA->>ParticipantB: TBD
~~~

no_diagram_rationale: TBD

## Threat, trust, data, and abuse cases

- Trust boundary: TBD
- Sensitive data or secret handling: TBD
- Abuse case and control: TBD

## Failure, security, and observability

| Failure or risk | Detection threshold | Handling or recovery |
|---|---|---|
| TBD | TBD | TBD |

## Migration and reconciliation

- Backup or restore point: TBD
- Rehearsal: TBD
- Reconciliation: TBD
- Irreversible boundary: TBD

## Test portfolio and TDD slices

Outer acceptance or contract oracle: TBD

1. RED: pending
2. GREEN: pending
3. REFACTOR: pending
4. CHECK: pending

Special evidence: security／performance／migration／availability as applicable: TBD

## Traceability

| Requirement | Acceptance | Test | Implementation | Evidence |
|---|---|---|---|---|
| REQ-001 | AC-001 | TBD | TBD | pending |

## Staged rollout, monitoring, and rollback

- Stage and stop condition: TBD
- Monitoring and threshold: TBD
- Rollback trigger and command or procedure: TBD
- Independent review or approval boundary: TBD

## Verification evidence

- RED evidence: pending
- GREEN evidence: pending
- Refactor or exception: pending
- Fresh verification: pending
- Realistic outcome check: pending
- Security／performance／migration evidence: pending
- Remaining risks: pending
