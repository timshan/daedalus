...
CHANGELOG.md impact: `[Unreleased]` carries the release-facing record, while `[1.0.1]` records the public installation repair and `[1.0.0]` records the baseline Plugin release.

Changelog evidence: repository-root `CHANGELOG.md` is present in the same release diff and the formal payload excludes repository documentation.
